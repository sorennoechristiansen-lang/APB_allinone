    ...
    try:
        if DAILY_CACHE_FILE.exists():
            with DAILY_CACHE_FILE.open("r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict) and data.get("date") == today_key():
        ...
        pass
    return {"schema": "PORTEFOLJE_SIMULATOR_DAILY_CACHE_V1", "date": today_key(), "fx_rates": {}, "phase1": {}, "phase2": {}, "vix": {}}
...
def make_phase2_row(display_row, raw_row=None, fundamental=None, analyst_error=None):
    ...
    if _phase0_preview_mode:
        # Fase 0 er kun kontrolvisning af allerede hentede data. Ingen historik-
        # eller regnskabsskrivning her; den sker ved den egentlige dataopdatering.
        target_age_days, target_age_date_known = None, False
    else:
        target_age_days, target_age_date_known = target_age_days_for_row(display_row, absolute_targets, price)
        register_earnings_date_for_row(display_row, fundamental.get("earnings_next_date"))
...
def _builder_report_progress(text, force=False):
    """Opdatér workerens seneste status uden at kalde Tkinter fra worker-tråden."""
...
def _builder_target_trend_for_key(key, history_data=None):
    """Blød kursmålstrend fra de to seneste reelle, komplette historikpunkter.

    Absolutte Bear/Base/Bull-mål sammenlignes procentvist. Base vægter 50 %,
    Bear og Bull 25 % hver. Har aktien færre end to komplette historikpunkter,
    behandles den neutralt (trend 0 %, score 50) og bliver ikke straffet.

    ``history_data`` kan gives af Fase 0, så hele aktieuniverset bruger én
    indlæsning af historikfilen i stedet for én filindlæsning pr. aktie.
    """
    try:
        data = history_data if isinstance(history_data, dict) else load_target_age_history()
        entry = data.get("positions", {}).get(str(key or "").upper(), {})
        hist = _complete_history_entries(entry.get("history", [])) if isinstance(entry, dict) else []
        if len(hist) < 2:
            return 0.0, 50.0, False
        previous, latest = hist[-2], hist[-1]
        changes = {}
        for field in ("bear_target", "base_target", "bull_target"):
            old = parse_float(previous.get(field), None)
            new = parse_float(latest.get(field), None)
            if old is None or old <= 0 or new is None or new <= 0:
                return 0.0, 50.0, False
            changes[field] = (new / old - 1.0) * 100.0
        trend_pct = 0.25 * changes["bear_target"] + 0.50 * changes["base_target"] + 0.25 * changes["bull_target"]
        score = clamp(50.0 + 2.5 * trend_pct, 0.0, 100.0)
        return trend_pct, score, True
    except Exception:
        return 0.0, 50.0, False
...
def _builder_candidate_rows():
    ...
    for row in rows_local:
        ...
        trend_pct, trend_score, trend_known = _builder_target_trend_for_key(_row_key(row))
        candidates.append({
            "row": row,
            "key": _row_key(row),
...
# ============================================================================
# APB ALL-IN-ONE WEB EDITION v0.1
...
.apb-card {border:1px solid rgba(49,51,63,.18);border-radius:12px;padding:12px 14px;background:rgba(250,250,250,.75);min-height:86px}
.apb-kicker {font-size:.75rem;color:#6b7280;text-transform:uppercase;letter-spacing:.03em}
.apb-big {font-size:1.45rem;font-weight:700;line-height:1.25}
.apb-subtle {color:#6b7280;font-size:.86rem}
</style>
...
def order_to_settings(order):
    ...
    t = order.get("price_target_trend",{}) or {}
    s["use_target_trend"] = bool(t.get("enabled",True))
...
def run_engine(order, progress=None):
    """Exact portfolio selection/capital optimisation chain from desktop v0.43, without Tkinter."""
    global _builder_current_candidates_context
...
def make_order_from_ui(mode="simple"):
    ...
    return {
        ...
        "priorities":{"Base 1Y":40.0,"Stock score":30.0,"Structure layer":15.0,"Sectors":10.0,"Industry":7.0,"Regions":5.0,"Price target trend":10.0,"Dividend":0.0},
        "sectors":sectors,"industry_preferences":{},"regions":regions,"structure_layers":structure_web,
        "price_target_trend":{"enabled":True},"dividend":{"enabled":False,"target_pct":2.0},
...
def show_result(result):
    ...
    st.success("Portfolio built successfully in the same web program.")
    total=sum(_safe(p.get("position_value")) for p in positions)+_safe(portfolio.get("cash",{}).get("amount"))
    weighted=lambda field: (sum(_safe(p.get("position_value"))*_safe(p.get(field)) for p in positions)/sum(_safe(p.get("position_value")) for p in positions)) if sum(_safe(p.get("position_value")) for p in positions) else None
    c1,c2,c3,c4=st.columns(4)
    with c1:_card("Portfolio value",f"{_fmt(total,0)} {ccy}",f"{len(positions)} positions")
    with c2:_card("Cash",f"{_fmt(portfolio.get('cash',{}).get('amount'),0)} {ccy}","")
    with c3:_card("Weighted Base 1Y",_pct(weighted("base_1y_pct")),"Analyst base case")
    with c4:_card("Engine score",_fmt(result.get("engine",{}).get("score"),2),"Desktop builder logic")
    st.subheader("Built portfolio")
    rows=[]
    for p in sorted(positions,key=lambda x:_safe(x.get("portfolio_weight_pct")),reverse=True):
        rows.append({"Name":p.get("name"),"Ticker":p.get("ticker"),"Exchange":p.get("exchange"),"Shares":_fmt(p.get("shares"),0),"Value":f"{_fmt(p.get('position_value'),0)} {ccy}","%PF":_pct(p.get("portfolio_weight_pct")),"Structure":p.get("structure_layer"),"Sector":p.get("sector"),"Bear 1Y":_pct(p.get("bear_1y_pct")),"Base 1Y":_pct(p.get("base_1y_pct")),"Bull 1Y":_pct(p.get("bull_1y_pct"))})
    st.dataframe(pd.DataFrame(rows),use_container_width=True,height=min(700,100+35*max(1,len(rows))))
    ...
    for tab,key,label in [(tabs[0],"structure_layers","Structure layers"),(tabs[1],"sectors","Sectors"),(tabs[2],"regions","Regions")]:
        with tab:
            data=phase3.get(key,[])
            if data:
                df=pd.DataFrame(data)[["category","portfolio_weight_pct","recommended_pct","deviation_pp","positions"]]
...
def admin_panel():
    st.subheader("Admin · server data")
    configured=os.environ.get("APB_ADMIN_PASSWORD","")
    if not configured:
        st.warning("Set environment variable APB_ADMIN_PASSWORD on the server to enable admin uploads.")
        return
    pw=st.text_input("Admin password",type="password")
    if pw!=configured:
        st.info("Enter admin password.")
        return
    st.caption("For this proof of concept the engine uses the same JSON files as the desktop builder. Admin can upload JSON files up to 10 MB; uploading replaces the server copy. Builder users cannot upload files.")
    targets=[("Stock universe","aktieunivers.json"),("Daily analysis cache","portefolje_dagsdata_cache.json"),("Price target history","portefolje_kursmaal_alder.json")]
    for label,fn in targets:
        col1,col2=st.columns([3,1])
        with col1: up=st.file_uploader(label,type=["json"],key=f"up_{fn}")
        with col2:
            p=Path.cwd()/fn
            st.caption(f"{'✓' if p.exists() else '–'} {fn}")
        if up is not None:
            try:
                raw = up.getvalue()
                ...
                tmp=(Path.cwd()/(fn+'.tmp')); tmp.write_text(json.dumps(obj,ensure_ascii=False,indent=2),encoding='utf-8'); tmp.replace(Path.cwd()/fn)
...
if "app_mode" not in st.session_state: st.session_state.app_mode="Builder"

head1,head2=st.columns([8,1])
with head1:
    st.title("Alpha Portfolio Builder")
    st.caption("All-in-one web proof of concept · Input → engine → result")
with head2:
    if LOGO_IMAGE_B64: st.markdown(_img(LOGO_IMAGE_B64,80),unsafe_allow_html=True)

mode=st.radio("",["Builder","Admin"],horizontal=True,label_visibility="collapsed")
if mode=="Admin":
    admin_panel(); st.stop()
...
if COVER_IMAGE_B64:
    _,mid,_=st.columns([1.4,4,1.4])
    with mid: st.markdown(_img(COVER_IMAGE_B64,694),unsafe_allow_html=True)

st.subheader("Build your portfolio")
experience=st.radio("Experience",["Simple","Advanced"],horizontal=True,help="Same engine. Advanced only exposes more controls.")

c1,c2,c3,c4=st.columns(4)
with c1: st.selectbox("Currency",["DKK","EUR","USD"],key="currency")
with c2: st.number_input("Portfolio value",min_value=1000.0,value=100000.0,step=10000.0,key="portfolio_value")
with c3: st.number_input("Minimum position",min_value=0.0,value=5000.0,step=1000.0,key="minimum_position")
...
if experience=="Simple":
    st.selectbox("Investment style",["Balanced","Growth","High growth"],key="simple_profile",help="This human-friendly choice is translated into the existing APB structure targets.")
    st.info("Simple mode deliberately hides the technical settings. It uses the same APB engine and standard sector/region targets.")
else:
    with st.expander("Structure layers",expanded=True):
        ...
        for col,(k,v) in zip(cols,{"Fundamental":60.0,"Growth":25.0,"Accelerator":10.0,"Potential":5.0}.items()):
            with col: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,key=f"str_{k}")
    with st.expander("Sector targets"):
        ...
        for i,(k,v) in enumerate(defaults.items()):
            with cols[i%3]: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,key=f"sec_{k}")
    with st.expander("Region targets"):
        ...
        cols=st.columns(3)
        for i,(k,v) in enumerate(defaults.items()):
            with cols[i%3]: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,key=f"reg_{k}")

st.text_input("Name (optional)",key="customer_name")
st.text_input("Skool username (optional for this proof of concept)",key="skool_username")
...
if st.button("BUILD PORTFOLIO",type="primary",use_container_width=True):
    ...
    messages=[]
    def progress(msg):
        messages.append(msg); status.info(msg); bar.progress(min(95,10+len(messages)*12))
    try:
        result=run_engine(order,progress)
        bar.progress(100); status.success("Portfolio ready")
        st.session_state.result_data=result
        st.rerun()
    except Exception as exc:
        bar.empty(); status.empty(); st.error(str(exc))
        st.caption("Admin can upload/update the JSON engine data under Admin.")
