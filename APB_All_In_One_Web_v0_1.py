        ...
        if users:
            ...
            for name in ordered_keys:
                ...
                rows.append({
                    ...
                    "Last login":e.get("last_login","") or "–",
                    "Last build":e.get("last_build","") or "–",
                })
...
def render_builder():
    ...
    if st.session_state.get("auth_tier") in ("Standard","Premium","VIP"):
        try:
            ...
            if quota_blocked:
                st.warning(
                    f"Portfolio builds this month: {quota_used} / {quota_limit} — "
                    f"Monthly limit reached. Resets {next_month.strftime('%B 1')}."
                )
            else:
            ...
            quota_blocked=True
            st.warning(
                "Build availability could not be verified at the moment. "
        ...
        try:
            ...
            if st.session_state.get("auth_tier") in ("Standard","Premium","VIP"):
                try:
                    ...
                    if "limit" in payload:
                        raise RuntimeError(
                            f"Monthly build limit reached ({payload.get('used',0)} / {payload.get('limit',0)})."
                        )
                    raise RuntimeError(
                        "The portfolio was calculated, but the build could not be registered securely. "
                        "Please try again."
                    ) from quota_exc
