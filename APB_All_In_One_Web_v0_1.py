...
def _build_portfolio_pdf(result):
    ...
    def distribution(title_text,key,explanation):
        ...
        if key=="structure_layers" and STRUCTURE_PYRAMID_B64:
            try:
                pyramid=Image(io.BytesIO(base64.b64decode(STRUCTURE_PYRAMID_B64)),width=49*mm,height=32.7*mm)
                figure=[Paragraph('Structure layer pyramid',small),Spacer(1,1*mm),pyramid]
                outer=Table([[tt,figure]],colWidths=[sum(smart_widths)+3*mm,54*mm],hAlign='LEFT')
