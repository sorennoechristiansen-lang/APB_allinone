...
def _apply_web_profile(prefix, selected_name, profiles, high_priority_key=None, auto_high_priority=False, all_keys=None):
    """Apply a preset only when the dropdown changes; users may edit every value afterwards."""
    marker=f"_{prefix}_profile_applied"
    if st.session_state.get(marker)==selected_name:
        return
    selected=dict(profiles.get(selected_name,{}) or {})
    keys=list(all_keys if all_keys is not None else selected.keys())
    for key in keys:
...
def _build_portfolio_pdf(result):
    ...
    story += [t,Paragraph('Built portfolio',h2),Paragraph('Positions are shown by model portfolio weight. Bear, Base and Bull are analyst 1-year scenarios used as inputs — not guaranteed outcomes.',small)]
    ...
    if STRUCTURE_PYRAMID_B64:
        try:
            pyramid=Image(io.BytesIO(base64.b64decode(STRUCTURE_PYRAMID_B64)),width=88*mm,height=58.7*mm)
            pyramid.hAlign='LEFT'
            story.append(KeepTogether([
                Paragraph('Understanding structure layers',h2),
                Paragraph('Structure layers are the portfolio\'s risk architecture. Fundamental positions form the stable base, while Growth, Accelerator and Potential progressively increase expected variability and risk.',body),
                pyramid,
            ]))
        except Exception:
            pass
    ...
    distribution('Structure layers','structure_layers','Actual allocation compared with the structure targets selected for this build.')
...
def render_builder():
    ...
    st.caption("Set the portfolio size and risk architecture first. APB then searches the available universe for the strongest overall fit to your choices.")
    experience=st.radio("Experience",["Simple","Advanced"],horizontal=True,help="Simple keeps the technical settings out of the way. Advanced exposes preset-driven controls that can still be edited manually.")

    c1,c2,c3,c4=st.columns(4)
    with c1: st.selectbox("Currency",["DKK","EUR","USD"],key="currency",help="Currency used for portfolio value, minimum position and the final report.")
    with c2: st.text_input("Portfolio value",value="100.000",key="portfolio_value_display",help="Total capital APB should allocate, e.g. 100.000.",on_change=_normalize_money_widget,args=("portfolio_value_display",100000))
    with c3: st.text_input("Minimum position",value="5.000",key="minimum_position_display",help="Smallest permitted position value. A higher minimum reduces the number of possible positions.",on_change=_normalize_money_widget,args=("minimum_position_display",5000))
    with c4: st.number_input("Maximum stocks",min_value=1,max_value=100,value=15,step=1,key="maximum_stocks",help="Upper limit for the number of stock positions in the portfolio.")
    st.number_input("Minimum stocks per active sector",min_value=0,max_value=10,value=0,step=1,key="minimum_sector",help="Optional diversification rule. Zero lets the optimiser decide freely.")

    if experience=="Simple":
        st.selectbox("Investment style",["Balanced","Growth","High growth"],key="simple_profile",help="The style is translated into the structure-layer mix shown below. Sector and region targets remain balanced defaults.")
        st.info("Simple mode keeps the setup short while using the same APB engine and market universe as Advanced mode.")
        _show_structure_pyramid(width=455)
    else:
        st.info("Advanced mode starts from ready-made profiles. Choose a preset, then change any individual percentage you want. High priority gives that target group much more influence in the optimisation.")
        ...
        with st.expander("Structure layers",expanded=True):
            ...
            structure_profile=st.selectbox("Structure profile",list(STRUCTURE_TARGET_PROFILES),index=0,key="structure_profile")
            ...
            for col,(k,v) in zip(cols,STRUCTURE_TARGET_PROFILES["Balanced"].items()):
                with col: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=1.0,format="%.0f",key=f"str_{k}")
        ...
        with st.expander("Sector targets"):
            ...
            sector_profile=st.selectbox("Sector profile",list(SECTOR_TARGET_PROFILES),index=0,key="sector_profile")
            ...
            for i,(k,v) in enumerate(SECTOR_TARGET_PROFILES["Balanced"].items()):
                with cols[i%3]: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=1.0,format="%.0f",key=f"sec_{k}")
        ...
        with st.expander("Region targets"):
            ...
            region_profile=st.selectbox("Region profile",list(REGION_TARGET_PROFILES),index=0,key="region_profile")
            ...
            for i,(k,v) in enumerate(REGION_TARGET_PROFILES["Global balanced"].items()):
                with cols[i%3]: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=1.0,format="%.0f",key=f"reg_{k}")
        ...
        with st.expander("Industry preferences"):
            ...
            industry_profile=st.selectbox("Industry preset",list(INDUSTRY_PREFERENCE_PROFILES),index=0,key="industry_profile")
