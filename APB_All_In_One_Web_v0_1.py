...
# ============================================================================
# APB ALL-IN-ONE WEB EDITION v0.4
...
def make_order_from_ui(mode="simple"):
    ...
    return {
        "type":"apb_order","schema_version":"web-0.1","name":st.session_state.get("customer_name",""),"skool_username":st.session_state.get("skool_username",""),"email":"",
...
def login_screen():
    if COVER_IMAGE_B64:
        _,mid,_=st.columns([1.6,3.4,1.6])
        with mid: st.markdown(_img(COVER_IMAGE_B64,600),unsafe_allow_html=True)
    st.markdown('<div class="apb-welcome"><b>Welcome to Alpha Portfolio Builder</b><br>Sign in to continue. Premium gives access to both Simple and Advanced building modes.</div>',unsafe_allow_html=True)
    c1,c2,c3=st.columns([1,1.15,1])
    with c2:
        access=st.radio("Access",["Standard","Premium","Admin"],horizontal=True,key="login_access")
...
def show_result(result):
    ...
    rows=[]
    for p in sorted(positions,key=lambda x:_safe(x.get("portfolio_weight_pct")),reverse=True):
        rows.append({"Name":p.get("name"),"Ticker":p.get("ticker"),"Exchange":p.get("exchange"),"Shares":_fmt(p.get("shares"),0),"Value":f"{_money(p.get('position_value'))} {ccy}","%PF":_pct(p.get("portfolio_weight_pct"),1),"Structure":p.get("structure_layer"),"Sector":p.get("sector"),"Bear 1Y":_pct(p.get("bear_1y_pct"),1),"Base 1Y":_pct(p.get("base_1y_pct"),1),"Bull 1Y":_pct(p.get("bull_1y_pct"),1)})
    ...
    with tabs[3]:
        ...
        st.download_button("Download result JSON",json.dumps(result,ensure_ascii=False,indent=2),file_name=f"{result.get('order_number','APB')}_Result.json",mime="application/json")
...
def admin_panel():
    ...
    with user_tab:
        ...
        with st.expander("Create user",expanded=not bool(users)):
            a,b,c=st.columns(3)
            with a: new_user=st.text_input("Username",key="new_user")
            with b: new_tier=st.selectbox("Tier",["Standard","Premium"],key="new_tier")
            with c: new_pw=st.text_input("Password",type="password",key="new_pw")
            if st.button("Create user",type="primary"):
                key=_safe_username(new_user)
                if not key or len(new_pw)<4: st.error("Use a username and a password of at least 4 characters.")
                elif key in users: st.error("Username already exists.")
                else:
                    salt,digest=_hash_password(new_pw)
                    users[key]={"tier":new_tier,"active":True,"salt":salt,"password_hash":digest,"created_at":datetime.now().astimezone().isoformat(timespec="seconds"),"last_login":"","login_count":0,"last_build":"","has_saved_data":False}
                    _save_users(users_data); st.success(f"Created {key} ({new_tier})."); st.rerun()
        if users:
            rows=[]
            for name,e in sorted(users.items()):
                rows.append({"Username":name,"Tier":e.get("tier"),"Active":bool(e.get("active",True)),"Logins":int(e.get("login_count",0) or 0),"Last login":e.get("last_login","") or "–","Last build":e.get("last_build","") or "–","Saved data":"Yes" if e.get("has_saved_data") else "No"})
            st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
            chosen=st.selectbox("Manage user",["–"]+sorted(users),key="manage_user")
            if chosen!="–":
                e=users[chosen]; c1,c2,c3=st.columns(3)
                with c1: tier=st.selectbox("Tier",["Standard","Premium"],index=0 if e.get("tier")=="Standard" else 1,key="manage_tier")
...
def render_builder():
    ...
    st.text_input("Name (optional)",key="customer_name")
    st.text_input("Skool username (optional)",key="skool_username")
    if st.button("BUILD PORTFOLIO",type="primary",use_container_width=True):
        ...
        try:
            ...
            if st.session_state.get("auth_tier")=="Premium":
...
with head2:
    ...
    st.markdown(f'<div class="apb-userline">{st.session_state.auth_user} · {st.session_state.auth_tier}</div>',unsafe_allow_html=True)
    if st.button("Log out",use_container_width=True):
        for k in ("auth_user","auth_tier","result_data"): st.session_state[k]=None
        st.rerun()

if st.session_state.auth_tier=="Standard":
    st.markdown('<div class="apb-welcome"><b>Standard access</b><br>Your account is active. The Portfolio Builder itself is part of Premium, where both Simple and Advanced modes are available.</div>',unsafe_allow_html=True)
    st.caption(DISCLAIMER); st.stop()

if st.session_state.auth_tier=="Admin":
    view=st.radio("",["Administration","Premium builder"],horizontal=True,label_visibility="collapsed")
