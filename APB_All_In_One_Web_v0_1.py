...
def render_builder():
    ...
    # Currency and available capital belong together; the remaining construction rules
    # are shown separately because each one affects the optimiser in a different way.
    ...
    with top2:
        st.text_input(
            f"Capital available to build the portfolio ({selected_currency})",
            value="100.000",
    ...
    st.text_input(
        f"Minimum value for one stock position ({selected_currency})",
        value="5.000",
        key="minimum_position_display",
        help="The smallest amount APB may allocate to a single stock. A higher minimum means fewer different positions can fit in the portfolio.",
        on_change=_normalize_money_widget,
