...
# v0.24: Retter langsom/blokerende opstart fra v0.23. GUI vises før cache-genopbygning, og Fase 0-preview genbruger én indlæst historik uden at skrive kursmål/regnskabsbegivenheder pr. aktie.
...
# Match sker entydigt på børs+ticker, mens layout og progress-visning fra v0.14 bevares.
...
# v0.11: Udbytteoversigten synkroniseres altid med den senest byggede portefølje.
...
# skrive til historikfilen for hver enkelt aktie. Det gjorde v0.23-opstarten
...
def fetch(force_refresh=False):
    ...
    # v0.23: beregn scorerne på den faktiske færdigbyggede DKK-fordeling.
...
def _builder_update_universe_data_worker(force_refresh=False):
    ...
    for idx, item in enumerate(work_items, 1):
        ...
        # v0.23: Historikken tilhører aktien – ikke aktieuniversfilen eller en konkret
...
def _builder_values_to_whole_shares(selected, desired_values, settings):
    ...
    while invested > target_value + 1e-6:
        ...
        if not choices:
            raise ValueError("De optimerede minimumspositioner kan ikke rummes inden for porteføljeværdien.")
...
# ===== Porteføljebygger v0.23: historikgrafer og Fase 0-data =====
...
# ============================================================================
# APB ALL-IN-ONE WEB EDITION v0.7
...
DISCLAIMER = ("Alpha Portfolio Builder is an analytical and educational tool and does not provide financial, "
              "investment, tax or legal advice. Data and calculations may contain errors, delays or omissions. "
              "Always verify relevant information independently.")
...
def run_engine(order, progress=None):
    ...
    return {
        ...
        "phase3":phase3,
        "engine":{"score":round(final_metrics.get("score",0),4),"base_1y":round(final_metrics.get("base_1y",0),4),"selection_passes":passes,"swaps":swaps,"capital_passes":capital_passes,"capital_moves":capital_moves},
    }
...
def make_order_from_ui(mode="simple"):
    # session-state values are already generated by widgets below.
    currency=st.session_state.get("currency","DKK")
    value=float(st.session_state.get("portfolio_value",100000.0))
    minpos=float(st.session_state.get("minimum_position",5000.0))
    maxstocks=int(st.session_state.get("maximum_stocks",15))
    ...
    if mode=="simple":
        ...
        sectors={"Technology":35.0,"Financials":9.0,"Healthcare":15.0,"Industrials":6.0,"Consumer Cyclical":10.0,"Consumer Defensive":5.0,"Energy":6.0,"Materials":6.0,"Utilities":3.0,"Transportation":2.0,"Communication":3.0,"Other sectors":0.0}
        regions={"USA":45.0,"Canada":5.0,"Denmark":5.0,"Other Nordics":5.0,"Europe":20.0,"Japan":5.0,"China / Hong Kong":4.0,"Other Asia":6.0,"Emerging Markets":3.0,"Other countries":2.0}
    else:
        structure_web={k:float(st.session_state.get(f"str_{k}",v)) for k,v in {"Fundamental":60,"Growth":25,"Accelerator":10,"Potential":5}.items()}
        sectors={k:float(st.session_state.get(f"sec_{k}",v)) for k,v in {"Technology":35,"Financials":9,"Healthcare":15,"Industrials":6,"Consumer Cyclical":10,"Consumer Defensive":5,"Energy":6,"Materials":6,"Utilities":3,"Transportation":2,"Communication":3,"Other sectors":0}.items()}
        regions={k:float(st.session_state.get(f"reg_{k}",v)) for k,v in {"USA":45,"Canada":5,"Denmark":5,"Other Nordics":5,"Europe":20,"Japan":5,"China / Hong Kong":4,"Other Asia":6,"Emerging Markets":3,"Other countries":2}.items()}
    return {
        "type":"apb_order","schema_version":"web-0.1","name":_current_display_username(),"skool_username":_current_display_username(),"email":"",
        ...
        "priorities":{"Base 1Y":40.0,"Stock score":30.0,"Structure layer":15.0,"Sectors":10.0,"Industry":7.0,"Regions":5.0,"Price target trend":0.0,"Dividend":0.0},
        "sectors":sectors,"industry_preferences":{},"regions":regions,"structure_layers":structure_web,
        "price_target_trend":{"enabled":False},"dividend":{"enabled":False,"target_pct":2.0},
...
def _load_users():
    base={"schema":"APB_USERS_V1","users":{}}
    try:
...
def _save_users(data):
    data=dict(data or {}); data["schema"]="APB_USERS_V1"; data.setdefault("users",{})
...
def _record_login(username):
    data=_load_users(); entry=data.get("users",{}).get(username)
    if not isinstance(entry,dict): return
    entry["last_login"]=datetime.now().astimezone().isoformat(timespec="seconds")
    entry["login_count"]=int(entry.get("login_count",0) or 0)+1
    _save_users(data)
...
def _save_premium_user_data(username, order, result):
    ...
    payload={"schema":"APB_USER_DATA_V1","username":username,"saved_at":datetime.now().astimezone().isoformat(timespec="seconds"),"latest_order":order,"latest_result":result}
    tmp=USER_DATA_DIR/(username+".tmp")
    dest=USER_DATA_DIR/(username+".json")
    tmp.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8"); tmp.replace(dest)
    users=_load_users(); entry=users.get("users",{}).get(username)
    if isinstance(entry,dict):
        entry["last_build"]=payload["saved_at"]
        entry["has_saved_data"]=True
        _save_users(users)
...
def login_screen():
    ...
    # One compact login panel instead of a full-width welcome banner.
    left,mid,right=st.columns([1.35,1.5,1.35])
    with mid:
        with st.container(border=True):
            st.markdown('<div class="apb-login-title">Welcome to Alpha Portfolio Builder</div><div class="apb-login-subtitle">Sign in to access your portfolio builder.</div>',unsafe_allow_html=True)
            access=st.radio("Access",["Standard","Premium","VIP","Admin"],horizontal=True,key="login_access")
            username=st.text_input("Username",value=("admin" if access=="Admin" else ""),key="login_username")
            password=st.text_input("Password",type="password",key="login_password")
            if st.button("LOG IN",type="primary",use_container_width=True):
                if access=="Admin":
                    configured=_configured_admin_password()
                    if username.strip().lower()=="admin" and configured and hmac.compare_digest(password,configured):
                        st.session_state.auth_user="admin"; st.session_state.auth_tier="Admin"; st.rerun()
                    st.error("Incorrect admin login.")
                else:
                    users=_load_users().get("users",{})
                    key=_find_user_key(users,username)
                    entry=users.get(key) if key is not None else None
                    if not isinstance(entry,dict) or not _verify_password(password,entry):
                        st.error("Incorrect username or password.")
                    elif str(entry.get("tier","Standard")) != access:
                        st.error(f"This account is registered as {entry.get('tier','Standard')}.")
                    elif not bool(entry.get("active",True)):
                        st.error("This account is inactive.")
                    else:
                        _record_login(key)
                        st.session_state.auth_user=key; st.session_state.auth_tier=access; st.rerun()



def _build_portfolio_pdf(result):
    """Create a clean customer-facing APB portfolio report as PDF bytes."""
    ...
    small=ParagraphStyle('APBSmall',parent=styles['Normal'],fontSize=7.5,leading=10,textColor=colors.HexColor('#52606d'))
    ...
    story += [Paragraph(f"{order} &nbsp;&nbsp; | &nbsp;&nbsp; Generated: {created}",sub),Spacer(1,4*mm)]

    summary=[
        ['Portfolio value',f"{_money(total)} {ccy}",'Positions',str(len(positions))],
        ['Cash',f"{_money(cash)} {ccy}",'Weighted Base 1Y',_pct(weighted('base_1y_pct'),1)],
        ['Engine score',_fmt((result.get('engine',{}) or {}).get('score'),1),'Currency',str(ccy)],
    ]
    t=Table(summary,colWidths=[35*mm,45*mm,38*mm,45*mm])
    t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#f5f8fc')),('BOX',(0,0),(-1,-1),0.5,colors.HexColor('#c9d7e8')),('INNERGRID',(0,0),(-1,-1),0.25,colors.HexColor('#dce5ef')),('FONTNAME',(0,0),(-1,-1),'Helvetica'),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('FONTNAME',(2,0),(2,-1),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6)]))
    story += [t,Paragraph('Built portfolio',h2)]

    headers=['#','Name','Ticker','Exchange','Shares',f'Value ({ccy})','%PF','Structure','Sector','Bear 1Y','Base 1Y','Bull 1Y']
    data=[headers]
    for i,x in enumerate(sorted(positions,key=lambda z:_safe(z.get('portfolio_weight_pct')),reverse=True),1):
        data.append([i,str(x.get('name') or ''),str(x.get('ticker') or ''),str(x.get('exchange') or ''),_fmt(x.get('shares'),0),_money(x.get('position_value')),_pct(x.get('portfolio_weight_pct'),1),str(x.get('structure_layer') or ''),str(x.get('sector') or ''),_pct(x.get('bear_1y_pct'),1),_pct(x.get('base_1y_pct'),1),_pct(x.get('bull_1y_pct'),1)])
    widths=[7,35,16,18,13,23,13,24,28,17,17,17]
    pt=Table(data,repeatRows=1,colWidths=[w*mm for w in widths])
    pt.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor('#0b4f9c')),('TEXTCOLOR',(0,0),(-1,0),colors.white),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTNAME',(0,1),(-1,-1),'Helvetica'),('FONTSIZE',(0,0),(-1,-1),7),('GRID',(0,0),(-1,-1),0.25,colors.HexColor('#d7e0ea')),('ROWBACKGROUNDS',(0,1),(-1,-1),[colors.white,colors.HexColor('#f7f9fc')]),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ALIGN',(0,0),(0,-1),'CENTER'),('ALIGN',(4,1),(6,-1),'RIGHT'),('ALIGN',(9,1),(11,-1),'RIGHT'),('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),4)]))
    story.append(pt)

    def distribution(title_text,key):
        rows=phase3.get(key,[]) or []
        if not rows: return
        story.append(Paragraph(title_text,h2))
        d=[['Category','Actual %','Target %','Deviation pp','Positions']]
        for r in rows:
            d.append([str(r.get('category') or ''),_fmt(r.get('portfolio_weight_pct'),1),_fmt(r.get('recommended_pct'),1),_fmt(r.get('deviation_pp'),1),str(r.get('positions') or '')])
        tt=Table(d,repeatRows=1,colWidths=[60*mm,28*mm,28*mm,30*mm,25*mm])
        ...
        story.append(tt)
    distribution('Structure layers','structure_layers')
    distribution('Sector allocation','sectors')
    distribution('Regional allocation','regions')
    story += [Spacer(1,4*mm),Paragraph('<b>Disclaimer</b>',small),Paragraph('Alpha Portfolio Builder provides model-based information and suggestions for educational purposes only. Investment decisions remain your own.',small)]
    doc.build(story)
    return buf.getvalue()
...
def show_result(result):
    ...
    with c3:_card("Weighted Base 1Y",_pct(weighted("base_1y_pct"),1),"Analyst base case")
    with c4:_card("Engine score",_fmt(result.get("engine",{}).get("score"),1),"Portfolio optimisation")
    ...
    for position_no,p in enumerate(sorted_positions,start=1):
        rows.append({"#":position_no,"Name":p.get("name"),"Ticker":p.get("ticker"),"Exchange":p.get("exchange"),"Shares":_fmt(p.get("shares"),0),"Value":f"{_money(p.get('position_value'))} {ccy}","%PF":_pct(p.get("portfolio_weight_pct"),1),"Structure":p.get("structure_layer"),"Sector":p.get("sector"),"Bear 1Y":_pct(p.get("bear_1y_pct"),1),"Base 1Y":_pct(p.get("base_1y_pct"),1),"Bull 1Y":_pct(p.get("bull_1y_pct"),1)})
    st.dataframe(pd.DataFrame(rows),use_container_width=True,height=min(700,100+35*max(1,len(rows))),hide_index=True)
    tabs=st.tabs(["Structure","Sectors","Regions","Input / details"])
    ...
    with tabs[3]:
        st.json(result.get("original_input",{}),expanded=False)
    try:
        pdf_bytes=_build_portfolio_pdf(result)
        st.download_button("Download Portfolio PDF",pdf_bytes,file_name=f"{result.get('order_number','APB')}_Portfolio.pdf",mime="application/pdf",type="primary")
    except Exception as exc:
...
def admin_panel():
    ...
    with user_tab:
        users_data=_load_users(); users=users_data.setdefault("users",{})
        ...
        with st.expander("Create user",expanded=not bool(users)):
            ...
            if st.button("Create user",type="primary",key=f"create_user_button_{nonce}"):
                ...
                if not display_name or len(new_pw)<4:
                    ...
                    users[key]={"display_name":display_name,"tier":new_tier,"active":True,"salt":salt,"password_hash":digest,"created_at":datetime.now().astimezone().isoformat(timespec="seconds"),"last_login":"","login_count":0,"last_build":"","has_saved_data":False}
        ...
        if users:
            ...
            for name in ordered_keys:
                ...
                rows.append({"Username":display_name,"Tier":e.get("tier"),"Active":bool(e.get("active",True)),"Logins":int(e.get("login_count",0) or 0),"Last login":e.get("last_login","") or "–","Last build":e.get("last_build","") or "–","Saved data":"Yes" if e.get("has_saved_data") else "No"})
            ...
            if chosen is not None:
                ...
                if st.button("Save user"):
                    e["tier"]=tier; e["active"]=active
                    if reset_pw:
                        salt,digest=_hash_password(reset_pw); e["salt"]=salt; e["password_hash"]=digest
                    _save_users(users_data); st.success("User updated."); st.rerun()
    with data_tab:
        st.caption("Upload server-side engine JSON files. Builder users cannot upload files. Maximum 10 MB each.")
        targets=[("Stock universe","aktieunivers.json"),("Daily analysis cache","portefolje_dagsdata_cache.json")]
        for label,fn in targets:
            col1,col2=st.columns([3,1])
            with col1: up=st.file_uploader(label,type=["json"],key=f"up_{fn}")
            with col2:
                p=Path.cwd()/fn; st.caption(f"{'✓' if p.exists() else '–'} {fn}")
            if up is not None:
                try:
                    raw=up.getvalue()
                    if len(raw)>10*1024*1024: raise ValueError("File is larger than the 10 MB admin upload limit.")
                    obj=json.loads(raw.decode("utf-8")); tmp=Path.cwd()/(fn+".tmp")
                    tmp.write_text(json.dumps(obj,ensure_ascii=False,indent=2),encoding="utf-8"); tmp.replace(Path.cwd()/fn)
                    st.success(f"Saved {fn}")
                except Exception as exc: st.error(f"Invalid JSON: {exc}")
...
def render_builder():
    if st.session_state.get("result_data") is not None:
        ...
        show_result(st.session_state.result_data); st.caption(DISCLAIMER); return
    if COVER_IMAGE_B64:
        _,mid,_=st.columns([1.5,4,1.5])
        with mid: st.markdown(_img(COVER_IMAGE_B64,660),unsafe_allow_html=True)
    st.subheader("Build your portfolio")
    st.caption("Choose the experience that suits you. Both modes use the same portfolio engine.")
    experience=st.radio("Experience",["Simple","Advanced"],horizontal=True,help="Simple keeps the technical settings out of the way. Advanced exposes the same Premium engine controls.")
    c1,c2,c3,c4=st.columns(4)
    with c1: st.selectbox("Currency",["DKK","EUR","USD"],key="currency")
    with c2: st.number_input("Portfolio value",min_value=1000.0,value=100000.0,step=10000.0,format="%.0f",key="portfolio_value")
    with c3: st.number_input("Minimum position",min_value=0.0,value=5000.0,step=1000.0,format="%.0f",key="minimum_position")
    with c4: st.number_input("Maximum stocks",min_value=1,max_value=100,value=15,step=1,key="maximum_stocks")
    st.number_input("Minimum stocks per active sector",min_value=0,max_value=10,value=0,step=1,key="minimum_sector")
    if experience=="Simple":
        st.selectbox("Investment style",["Balanced","Growth","High growth"],key="simple_profile",help="Translated automatically into APB structure targets.")
        st.info("Simple mode keeps the setup short while using the same APB engine and market universe.")
    else:
        with st.expander("Structure layers",expanded=True):
            cols=st.columns(4)
            for col,(k,v) in zip(cols,{"Fundamental":60.0,"Growth":25.0,"Accelerator":10.0,"Potential":5.0}.items()):
                with col: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"str_{k}")
        with st.expander("Sector targets"):
            defaults={"Technology":35.0,"Financials":9.0,"Healthcare":15.0,"Industrials":6.0,"Consumer Cyclical":10.0,"Consumer Defensive":5.0,"Energy":6.0,"Materials":6.0,"Utilities":3.0,"Transportation":2.0,"Communication":3.0,"Other sectors":0.0}
            cols=st.columns(3)
            for i,(k,v) in enumerate(defaults.items()):
                with cols[i%3]: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"sec_{k}")
        with st.expander("Region targets"):
            defaults={"USA":45.0,"Canada":5.0,"Denmark":5.0,"Other Nordics":5.0,"Europe":20.0,"Japan":5.0,"China / Hong Kong":4.0,"Other Asia":6.0,"Emerging Markets":3.0,"Other countries":2.0}
            cols=st.columns(3)
            for i,(k,v) in enumerate(defaults.items()):
                with cols[i%3]: st.number_input(k,min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"reg_{k}")
    if st.button("BUILD PORTFOLIO",type="primary",use_container_width=True):
        order=make_order_from_ui("simple" if experience=="Simple" else "advanced")
        status=st.empty(); detail=st.empty(); bar=st.progress(2)
        state={"p":2,"last":time.monotonic()}
        def progress(msg):
            # Internal optimiser reports now reach the web UI every ~0.2 s.
            lower=str(msg).lower()
            if "reading" in lower: base=5
            elif "start portfolio" in lower or "creating" in lower: base=12
            elif "aktieoptimering" in lower or "stock selection" in lower or "optimising stock" in lower: base=22
            elif "kapitaloptimering" in lower or "capital allocation" in lower or "optimising capital" in lower: base=62
            elif "whole shares" in lower or "converting" in lower: base=91
            else: base=state["p"]
            state["p"]=max(state["p"],min(96,base+1))
            status.info("Building your portfolio…")
            detail.caption(str(msg))
            bar.progress(int(state["p"]))
        try:
            result=run_engine(order,progress)
            bar.progress(100); status.success("Portfolio ready"); detail.empty()
            ...
            bar.empty(); status.empty(); detail.empty(); st.error(str(exc))
        finally:
...
with head2:
    if LOGO_IMAGE_B64: st.markdown(_img(LOGO_IMAGE_B64,76),unsafe_allow_html=True)
    st.markdown(f'<div class="apb-userline">{_current_display_username()} · {st.session_state.auth_tier}</div>',unsafe_allow_html=True)
