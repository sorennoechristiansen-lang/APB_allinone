...
def _allocation_controls(prefix, profiles):
    ...
    auto=st.checkbox(
        ...
        help="Keeps the value you edit and adjusts the other values so the total remains 100%.",
    )
    if auto:
        st.radio(
...
def render_builder():
    ...
    # Keep the basic build inputs compact and visible at a glance.
    # The sector-specific diversification rule now lives with Sector targets below.
    ...
    with top1:
        st.selectbox(
            ...
            ["DKK","EUR","USD"],
            key="currency",
            help="Currency used for all portfolio amounts and in the final report.",
        )
    selected_currency=st.session_state.get("currency","DKK")
    with top2:
        st.text_input(
            f"Capital available ({selected_currency})",
            value="100.000",
            key="portfolio_value_display",
            help="The total amount APB may use when constructing the portfolio.",
            on_change=_normalize_money_widget,
            args=("portfolio_value_display",100000),
        )
    with top3:
        st.text_input(
            f"Minimum per stock ({selected_currency})",
            value="5.000",
            key="minimum_position_display",
            help="The smallest amount APB may allocate to one stock position. A higher minimum means fewer different positions can fit in the portfolio.",
            on_change=_normalize_money_widget,
            args=("minimum_position_display",5000),
        )
    with top4:
        st.number_input(
            ...
            step=1,
            key="maximum_stocks",
            help="APB builds the portfolio using exactly this number of different stocks, provided the selected rules can be satisfied.",
        )
    ...
    structure_heading=f"Structure layers | {st.session_state.get('structure_profile','Balanced')}"
    with st.expander(structure_heading,expanded=(st.session_state.get("_open_target_section")=="structure")):
        ...
        structure_profile=st.selectbox("Structure profile",list(STRUCTURE_TARGET_PROFILES),index=0,key="structure_profile",on_change=_keep_target_section_open,args=("structure",))
        ...
        for col,(k,v) in zip(cols,STRUCTURE_TARGET_PROFILES["Balanced"].items()):
            with col:
                st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=0.1,format="%.1f",key=f"str_{k}",on_change=_rebalance_100,args=("str",structure_keys,k))
        _show_structure_pyramid(width=455,caption=False)

    sector_heading=f"Sector targets | {st.session_state.get('sector_profile','Balanced')}"
    with st.expander(sector_heading,expanded=(st.session_state.get("_open_target_section")=="sector")):
        ...
        sector_profile=st.selectbox("Sector profile",list(SECTOR_TARGET_PROFILES),index=0,key="sector_profile",on_change=_keep_target_section_open,args=("sector",))
        ...
        st.number_input(
            "Minimum stocks in each active sector",
            min_value=0,
            max_value=10,
            value=0,
            step=1,
        ...
        for i,(k,v) in enumerate(SECTOR_TARGET_PROFILES["Balanced"].items()):
            with cols[i%3]:
                st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=0.1,format="%.1f",key=f"sec_{k}",on_change=_rebalance_100,args=("sec",sector_keys,k))

    region_heading=f"Region targets | {st.session_state.get('region_profile','Global balanced')}"
    with st.expander(region_heading,expanded=(st.session_state.get("_open_target_section")=="region")):
        ...
        region_profile=st.selectbox("Region profile",list(REGION_TARGET_PROFILES),index=0,key="region_profile",on_change=_keep_target_section_open,args=("region",))
        ...
        for i,(k,v) in enumerate(REGION_TARGET_PROFILES["Global balanced"].items()):
            with cols[i%3]:
                st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,step=0.1,format="%.1f",key=f"reg_{k}",on_change=_rebalance_100,args=("reg",region_keys,k))
