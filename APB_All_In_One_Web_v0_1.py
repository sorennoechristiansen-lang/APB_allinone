...
# v0.9e: Allocation section headings now show the matching preset based on the actual values, or Custom setup when the values no longer match any preset.
...
def render_builder():
    ...
    structure_heading=f"Structure layers | {_matching_allocation_profile('str',STRUCTURE_TARGET_PROFILES)}"
    ...
    sector_heading=f"Sector targets | {_matching_allocation_profile('sec',SECTOR_TARGET_PROFILES)}"
    ...
    region_heading=f"Region targets | {_matching_allocation_profile('reg',REGION_TARGET_PROFILES)}"
