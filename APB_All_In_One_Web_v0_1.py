...
def make_order_from_ui(mode="simple"):
    ...
    value=_parse_eu_number(st.session_state.get("portfolio_value_display","100.000"),100000.0)
    minpos=_parse_eu_number(st.session_state.get("minimum_position_display","5.000"),5000.0)
    ...
    return {
        ...
        "priorities":{"Base 1Y":40.0,"Stock score":30.0,"Structure layer":15.0,"Sectors":10.0,"Industry":7.0,"Regions":(250.0 if (mode=="advanced" and bool(st.session_state.get("region_high_priority",False))) else 5.0),"Price target trend":0.0,"Dividend":0.0},
...
def _build_portfolio_pdf(result):
    ...
    for i,x in enumerate(sorted(positions,key=lambda z:_safe(z.get('portfolio_weight_pct')),reverse=True),1):
        data.append([i,_clip_pdf_text(x.get('name'),30),str(x.get('ticker') or ''),str(x.get('exchange') or ''),_fmt(x.get('shares'),0),_money(x.get('position_value')),_pct(x.get('portfolio_weight_pct'),1),_display_structure(x.get('structure_layer')),_clip_pdf_text(_display_sector(x.get('sector')),22),_pct(x.get('bear_1y_pct'),1),_pct(x.get('base_1y_pct'),1),_pct(x.get('bull_1y_pct'),1)])
    ...
    def distribution(title_text,key):
        ...
        # Width follows the widest visible value in each column, plus comfortable padding.
        # The main portfolio table above deliberately keeps its fixed widths.
        natural_widths=[]
        for col_idx in range(len(d[0])):
            ...
            natural_widths.append(widest + 16)  # points: text width + left/right breathing room
        # Sensible limits keep category names readable without letting any column become oversized.
        min_widths=[42*mm,20*mm,20*mm,24*mm,20*mm]
        max_widths=[68*mm,34*mm,34*mm,38*mm,30*mm]
        smart_widths=[
            max(min_widths[i],min(natural_widths[i],max_widths[i]))
            for i in range(len(natural_widths))
        ]
...
def show_result(result):
    ...
    for position_no,p in enumerate(sorted_positions,start=1):
        rows.append({"#":position_no,"Name":p.get("name"),"Ticker":p.get("ticker"),"Exchange":p.get("exchange"),"Shares":_fmt(p.get("shares"),0),"Value":f"{_money(p.get('position_value'))} {ccy}","%PF":_pct(p.get("portfolio_weight_pct"),1),"Structure":_display_structure(p.get("structure_layer")),"Sector":_display_sector(p.get("sector")),"Bear 1Y":_pct(p.get("bear_1y_pct"),1),"Base 1Y":_pct(p.get("base_1y_pct"),1),"Bull 1Y":_pct(p.get("bull_1y_pct"),1)})
    ...
    with tabs[3]:
        ...
        input_rows=[
            ...
            {"Setting":"Portfolio value","Value":f"{_money(basic.get('portfolio_value'))} {ccy}"},
            {"Setting":"Minimum position","Value":f"{_money(basic.get('minimum_position'))} {ccy}"},
            {"Setting":"Maximum positions","Value":_fmt(basic.get("maximum_number_of_stocks"),0)},
            {"Setting":"Minimum stocks per sector","Value":_fmt(basic.get("minimum_stocks_per_sector"),0)},
        ]
        st.dataframe(pd.DataFrame(input_rows),use_container_width=False,hide_index=True)
        ...
        st.dataframe(pd.DataFrame(result_rows),use_container_width=False,hide_index=True)
    try:
...
def render_builder():
    ...
    with c2: st.text_input("Portfolio value",value="100.000",key="portfolio_value_display",help="Use e.g. 100.000 for 100,000.",on_change=_normalize_money_widget,args=("portfolio_value_display",100000))
    with c3: st.text_input("Minimum position",value="5.000",key="minimum_position_display",help="Use e.g. 5.000 for 5,000.",on_change=_normalize_money_widget,args=("minimum_position_display",5000))
    ...
    if experience=="Simple":
        ...
        with st.expander("Structure layers",expanded=True):
            ...
            for col,(k,v) in zip(cols,{"Fundamental":60.0,"Growth":25.0,"Accelerator":10.0,"Potential":5.0}.items()):
                with col: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"str_{k}")
        with st.expander("Sector targets"):
            ...
            for i,(k,v) in enumerate(defaults.items()):
                with cols[i%3]: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"sec_{k}")
        with st.expander("Region targets"):
            st.checkbox(
                "High priority",
                value=False,
                key="region_high_priority",
                help="Give your regional targets much more influence in the optimization. Leave unchecked for normal APB balancing."
            )
            ...
            cols=st.columns(3)
            for i,(k,v) in enumerate(defaults.items()):
                with cols[i%3]: st.number_input(f"{k} (%)",min_value=0.0,max_value=100.0,value=v,step=1.0,format="%.0f",key=f"reg_{k}")
